...
TimelineSegmentDrawer::TimelineSegmentDrawer(int segmentNumber,
    ...
    : m_segmentNumber(segmentNumber), m_timelineLength(timelineLength), m_currentTime(currentTime), m_numberOfDivisions(numberOfDivisions), m_isAbsoluteTime(isAbsoluteTime), m_drawArea(drawArea), m_smoothOffset(0.0), m_segmentTime(), m_labelSet(false)
{
    // Calculate and store the segment time once during construction
    if (segmentNumber % 3 == 0)
    { // Only every 3rd segment gets a label
...
TimelineChevronDrawer::TimelineChevronDrawer(const QRect &drawArea, int yOffset)
    : m_drawArea(drawArea), m_yOffset(yOffset), m_chevronWidthPercent(0.4) // 40% of widget width
      ,
...
TimelineBackgroundDrawer::TimelineBackgroundDrawer(const QRect &drawArea, const QColor &backgroundColor)
    : m_drawArea(drawArea), m_backgroundColor(backgroundColor), m_borderColor(QColor(150, 150, 150)), m_borderWidth(1)
{
}
